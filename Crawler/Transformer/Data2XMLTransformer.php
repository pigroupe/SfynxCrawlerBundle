 ...
 *
 * Usage:
 *       $xml = Data2XMLTransformer::build('root_node_name', $php_array);
 *       echo $xml->saveXML();
 */
namespace Sfynx\CrawlerBundle\Crawler\Transformer;

class Data2XMLTransformer
{
     ...
     * @return \DomDocument
     */
    public static function &build($node_name, $arr = [])
    {
        $xml = self::getXMLRoot();
     ...
     * @return \DOMNode
     */
    protected static function &convert($node_name, $arr = [])
    {
        $xml  = self::getXMLRoot();
        ...
        if (!is_array($arr)) {
            $node->appendChild(
                $xml->createTextNode(self::bool2str($arr))
            );
        }
     ...
     */
    protected static function bool2str($v)
    {
        return $v === true || $v === false ? (bool)$v : $v;
    }
